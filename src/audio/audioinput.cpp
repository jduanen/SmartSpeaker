...
#undef G_LOG_DOMAIN
#define G_LOG_DOMAIN "smartspeaker::AudioInput"

smartspeaker::AudioInput::AudioInput(App *app)
...
smartspeaker::AudioInput::~AudioInput() { WebRtcVad_Free(vad_instance); }

void smartspeaker::AudioInput::close() {
 ...
 * audio input thread.
 */
void smartspeaker::AudioInput::wake() {
 ...
 * @return int32_t
 */
size_t smartspeaker::AudioInput::ms_to_frames(size_t frame_length, size_t ms) {
  //     floor <-  (samples/sec * (     seconds     )) / (samples/frame)
  g_assert(sample_rate > 0);
  return (size_t)((sample_rate * ((double)ms / 1000)) / frame_length);
}

void smartspeaker::AudioInput::transition(State to_state) {
...
void smartspeaker::AudioInput::loop_waiting() {
...
void smartspeaker::AudioInput::loop_woke() {
...
void smartspeaker::AudioInput::loop_listening() {
  AudioFrame new_frame = input->read_frame(AUDIO_INPUT_VAD_FRAME_LENGTH);
...
void smartspeaker::AudioInput::loop() {
  for (;;) {
    switch (state) {
      case State::CLOSED:
        return;
