...
void smartspeaker::AudioVolumeDriverAlsa::duck() {
  if (ducked)
    return;
...
void smartspeaker::AudioVolumeDriverAlsa::unduck() {
  if (!ducked)
    return;
...
void smartspeaker::AudioVolumeDriverAlsa::set_volume(int volume) {
...
int smartspeaker::AudioVolumeDriverAlsa::get_volume() {
  return get_volume(app->config->audio_output_device,
                    app->config->audio_volume_control);
}

int smartspeaker::AudioVolumeDriverAlsa::get_volume(const char *device_name,
                                             const char *ctl_name) {
  snd_mixer_t *handle = NULL;
