...
smartspeaker::AudioFIFO::AudioFIFO(App *appInstance) {
  app = appInstance;
  running = reading = false;
}

smartspeaker::AudioFIFO::~AudioFIFO() {
  running = false;
  if (fd > 0) {
    close(fd);
  }
...
int smartspeaker::AudioFIFO::init() {
  struct stat st;
...
void *smartspeaker::AudioFIFO::loop(gpointer data) {
...
bool smartspeaker::AudioFIFO::isReading() { return reading; }
