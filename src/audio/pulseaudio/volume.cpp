...
smartspeaker::AudioVolumeDriverPulseAudio::AudioVolumeDriverPulseAudio(App *app)
...
smartspeaker::AudioVolumeDriverPulseAudio::~AudioVolumeDriverPulseAudio() {
...
void smartspeaker::AudioVolumeDriverPulseAudio::duck() {
  if (ducking_stream)
    return;
...
void smartspeaker::AudioVolumeDriverPulseAudio::unduck() {
  if (!ducking_stream)
    return;
...
void smartspeaker::AudioVolumeDriverPulseAudio::set_volume(int volume) {
  struct pa_cvolume v;
  pa_cvolume_init(&v);
...
int smartspeaker::AudioVolumeDriverPulseAudio ::get_volume() {
  return convert_to_range(sink_volume, 0, PA_VOLUME_NORM,
                          AudioVolumeController::MIN_VOLUME,
                          AudioVolumeController::MAX_VOLUME);
}

void smartspeaker::AudioVolumeDriverPulseAudio::context_state_cb(pa_context *c,
                                                          void *userdata) {
...
void smartspeaker::AudioVolumeDriverPulseAudio::subscribe_cb(
...
void smartspeaker::AudioVolumeDriverPulseAudio::sink_info_cb(pa_context *c,
...
void smartspeaker::AudioVolumeDriverPulseAudio::volume_cb(pa_context *c, int success,
                                                   void *userdata) {
  if (success) {
    g_debug("Volume set\n");
  } else {
...
void smartspeaker::AudioVolumeDriverPulseAudio::set_volume(char *device, int channels,
                                                    int volume) {
  struct pa_cvolume v;
  pa_cvolume_init(&v);
...
void smartspeaker::AudioVolumeDriverPulseAudio::server_info_cb(pa_context *ctx,
