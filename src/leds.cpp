...
smartspeaker::Leds::Leds(App *appInstance) {
  app = appInstance;
  initialized = false;
...
void smartspeaker::Leds::animate(LedsState_t state) {
...
void smartspeaker::Leds::animate_internal(LedsAnimation_t style, int color,
                                   int duration) {
  switch (style) {
...
int smartspeaker::Leds::get_brightness_internal(bool max = false) {
...
void smartspeaker::Leds::solid(int color) {
...
void smartspeaker::Leds::clear(int color) {
  for (int i = 0; i < led_count; i++) {
    leds[i] = color;
  }
}

bool smartspeaker::Leds::set_leds() {
  char buffer[128];
